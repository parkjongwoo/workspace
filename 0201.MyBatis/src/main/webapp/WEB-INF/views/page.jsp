		...
		<nav aria-label="Page navigation example">
			<ul class="pagination  justify-content-center">				
				<li class="page-item <c:if test="${selectPage<=1}">disabled</c:if>">
			    	<a class="page-link" href="page?selectPage=${selectPage-1}" aria-label="Previous">
			        	<span aria-hidden="true">&laquo;</span>
			        	<span class="sr-only">Previous</span>
			      	</a>
			    </li>				
				<c:forEach varStatus="idx" begin="${firstPageNum}" end="${lastPageNum}">
					<li class="page-item <c:if test="${selectPage== idx.current}">active</c:if>"><a class="page-link" href="page?selectPage=${idx.current}">${idx.current}</a></li>
				</c:forEach>				
